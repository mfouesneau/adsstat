""" Adapted from Wordcloud from Andreas Christian Mueller <amueller@ais.uni-bonn.de> """
...
FONT_PATH = localpath + "/DroidSansMono.ttf"


def make_wordcloud(words, counts, font_path=None, width=600, height=300,
                   margin=5, ranks_only=False, color_from_level=False):
    ...
    color_from_level : boolean (default=False)
        If set, the color code follows the counts or the rank of the words
    ...
    if ranks_only:
        levels = np.asarray(font_sizes)
        levels = (levels * 360. / max(levels)).astype(int)
    else:
        levels = (counts * 360. / max(counts)).astype(int)
    everything = zip(words, font_sizes, positions, orientations, levels)
    for word, font_size, position, orientation, level in everything:
        font = ImageFont.truetype(font_path, font_size)
        # transpose font optionally
        transposed_font = ImageFont.TransposedFont(font, orientation=orientation)
        draw.setfont(transposed_font)
        if color_from_level:
            draw.text((position[1], position[0]), word, fill="hsl(%d" % level + ", 80%, 50%)")
        else:
            draw.text((position[1], position[0]), word, fill="hsl(%d" % random.randint(0, 360) + ", 80%, 50%)")
    #img.show()
    #img.save(fname)
    return img
